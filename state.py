...
class State(object):
	...
	def remove_utxo(self, utxo_id_arg, utxo_value_arg, utxo_owner_arg):
		'''Takes a TransactionInput and the sender's address and removes the respective UXTO'''
		del self.utxo[utxo_owner_arg][utxo_id_arg]
		self.wallet_sum[utxo_owner_arg] -= utxo_value_arg
	...
	def undo_transaction(self,tx):
		...
		for utxo_to_remove in tx.transaction_outputs:
			self.remove_utxo(utxo_to_remove.output_id, utxo_to_remove.value, utxo_to_remove.recipient_address)
